...
namespace Client.MirObjects
{
    public class PlayerObject : MapObject
    {
        ...
        public int Armour, Weapon, WeaponEffect, Shield,EquipShieldEffect, ArmourOffSet, HairOffSet, WeaponOffSet, WingOffset, MountOffset;
        ...
        public void Load(S.ObjectPlayer info)
        {
            ...
            Shield = info.Shield;
            EquipShieldEffect = info.EquipShieldEffect;
			Armour = info.Armour;
            Light = info.Light;
            ...
            else
            {
                ...
                switch (Class)
                {
                    ...
                    case MirClass.Wizard:
                       ...
                       // MirMessageBox.Show(Shield.ToString());
                        if (Shield > 0)
                        {
                            ShieldLibrary = Libraries.CShields[Shield - 1];
                           // if (WeaponEffect > 0)
                           //     WeaponEffectLibrary1 = WeaponEffect < Libraries.CWeaponEffect.Length ? Libraries.CWeaponEffect[WeaponEffect] : null;
                           // else
                           //     WeaponEffectLibrary1 = null;
                        ...
                        #region WingEffects
                        if (WingEffect > 0 && WingEffect < 254)
                        {
                            WingLibrary = (WingEffect - 1) < Libraries.CHumEffect.Length ? Libraries.CHumEffect[WingEffect - 1] : null;
                        }
                        #endregion
        ...
        public virtual void SetEffects()
        {
            ...
            if (WingEffect >= 250)
            {
                ...
                {
                    case 254:
                        Effects.Add(new SpecialEffect(Libraries.CHumEffect[147], 0, 8, 700, this, false, true, 0) { Repeat = true, LightColour = Color.Red, Light = 4 });
                        break;
                    case 255: //Oma King Robe effect
            ...
            //moha
            if (LevelEffects.HasFlag(LevelEffects.BlueFlame))
            {
                Effects.Add(new SpecialEffect(Libraries.Effect, 296, 12, 3000, this, true, false, 1) { Repeat = true });
                SpecialEffect effect = new SpecialEffect(Libraries.Effect, 312, 12, 4200, this, true, false, 1) { Repeat = true, Delay = delay };
                effect.SetStart(CMain.Time + delay);
                Effects.Add(effect);
            }
        }
        ...
        public override void Draw()
        {
            ...
            if (this == User)/*
                ...
                else if (MapObject.User.Direction == MirDirection.DownRight)
                {
                    if (this == User)
                        MapObject.User.DrawWings();
                    DrawShield();
                ...
                else if (      MapObject.User.Direction == MirDirection.Down)
                {
                ...
                else if (MapObject.User.Direction == MirDirection.DownLeft)
                {
                    if (this == User)
                        MapObject.User.DrawWings();
                    DrawBody();
                ...
                else if (MapObject.User.Direction == MirDirection.Left ||
                    MapObject.User.Direction == MirDirection.UpLeft)
                {
                    DrawBody();
                    MapObject.User.DrawHead();
            ...
            if (this != User)
                ...
                else if (Direction == MirDirection.Left ||
                    Direction == MirDirection.UpLeft)
                {
                    DrawBody();
                    DrawHead();
        ...
        {
            if (Armour >= 85 && Armour <= 145 || Armour == 164) return; // New standard armours
        ...
        public void DrawWeapon()
        {
            ...
            // Paul
            // For new main armours
            //if (Armour >= 85 && Armour <= 123) // 0 for test
            //{
            //  DrawLocation.X += AdjustedPlacements.ArmourSet2[DrawFrame + WeaponOffSet].X;
            //  DrawLocation.Y += AdjustedPlacements.ArmourSet2[DrawFrame + WeaponOffSet].Y;
            //}

            if (WeaponLibrary1 != null)
            {
                //WeaponLibrary1.Draw(DrawFrame + WeaponOffSet, DrawLocation, DrawColour, true); //original
                WeaponLibrary1.Draw(DrawFrame + WeaponOffSet, (Armour >= 85 && Armour <= 123) ? new Point(
                    DrawLocation.X + AdjustedPlacements.ArmourSet2[DrawFrame + WeaponOffSet].X,
                    DrawLocation.Y + AdjustedPlacements.ArmourSet2[DrawFrame + WeaponOffSet].Y) :
                    DrawLocation, DrawColour, true); //Paul
        ...
        {
            if (WingEffect <= 0 || WingEffect >= 250) return;
            // Paul
            if (WingLibrary != null)
                switch (WingEffect)
                {
                    ...
                    case 62:
                    case 64:
                        WingLibrary.DrawBlend(DrawFrame, DrawLocation, DrawColour, true);  // Armour Frames with Blend
                        break;
                    case 43:
                    case 44:
                    case 45:
                    ...
                    case 119:
                    case 120:
                        WingLibrary.Draw(DrawFrame, DrawLocation, DrawColour, true); // Armour Frames with Transparency
                        break;
                    case 63:
                    case 121:
                    case 122:
                    ...
                    case 146:
                    case 147:
                        WingLibrary.DrawBlend(DrawWingFrame, DrawLocation, DrawColour, true); // Wing Frames with Blend
                        break;
                    case 91:
                    case 92:
                    case 93:
                    ...
                    case 101:
                    case 102:
                        WingLibrary.DrawBlend(DrawFrame + ArmourOffSet, DrawLocation, DrawColour, true); // Armour Frames with Blend (M+F)
                        break;
                    case 126:
                        WingLibrary.Draw(DrawWingFrame, DrawLocation, DrawColour, true); // Wing Frames with Transparency
                        break;
                    default:
                        WingLibrary.DrawBlend(DrawWingFrame + WingOffset, DrawLocation, DrawColour, true); // Wing Frames with Blend (M+F)
                        break;
                }
        }
