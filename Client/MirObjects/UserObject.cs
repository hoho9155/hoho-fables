...
namespace Client.MirObjects
{
    public class UserObject : PlayerObject
    {
        ...
        private void RefreshLevelStats()
        {
            ...
            MaxBagWeight = /*(ushort)Math.Min(ushort.MaxValue, 50 + Level / CoreStats.BagWeightGain * Level)*/ ushort.MaxValue;
        ...
        private void RefreshEquipmentStats()
        {
            ...
            for (int i = 0; i < Equipment.Length; i++)
            {
                ...
                {
                    WingEffect = (WingEffect >0) ? WingEffect : RealItem.Effect;
                }
                ...
                {
                    Shield = RealItem.Shape;
                    EquipShieldEffect = RealItem.Effect;
                }
        ...
        private void RefreshMirSetStats()
        {
            if (MirSet.Count() == 10)
            {
                MaxAC = (ushort)Math.Min(ushort.MaxValue, MaxAC + 1);
                MaxMAC = (ushort)Math.Min(ushort.MaxValue, MaxMAC + 1);
                //MaxBagWeight = /*(ushort)Math.Min(ushort.MaxValue, MaxBagWeight + 70)*/ ushort.MaxValue;
        ...
        public void GetMaxGain(UserItem item)
        {
            ...
            if (item.Info.Type == ItemType.Amulet)
            {
                ...
                if (min == 0)
                {
                    GameScene.Scene.ChatControl.chatDialog.ReceiveChat(FreeSpace(Inventory) == 0 ? "You do not have enough space." : "You do not have enough weight.", ChatType.System);
            ...
            if (item.Info.StackSize > 1)
            {
                ...
                if (min == 0)
                {
                    GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You do not have enough space.", ChatType.System);
                    item.Count = 0;
                }
            }
            else
