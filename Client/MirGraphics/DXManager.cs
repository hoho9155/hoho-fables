...
namespace Client.MirGraphics
{
    ...
    class DXManager
    {
        ...
        public static Texture BossRadarTexture;
        ...
        private static unsafe void LoadTextures(byte LightVariable = 0)
        {
            ...
            {
                RadarTexture = new Texture(Device, Settings.MiniMapRadarSize, Settings.MiniMapRadarSize, 1, Usage.None, Format.A8R8G8B8, Pool.Managed);

                using (GraphicsStream stream = RadarTexture.LockRectangle(0, LockFlags.Discard))
                using (Bitmap image = new Bitmap(Settings.MiniMapRadarSize, Settings.MiniMapRadarSize, Settings.MiniMapRadarSize * 4, PixelFormat.Format32bppArgb, (IntPtr)stream.InternalDataPointer))
                using (Graphics graphics = Graphics.FromImage(image))
                    graphics.Clear(Color.White);
            }
            if (BossRadarTexture == null || BossRadarTexture.Disposed)
            {
                BossRadarTexture = new Texture(Device, (Settings.MiniMapRadarSize * 2), (Settings.MiniMapRadarSize * 2), 1, Usage.None, Format.A8R8G8B8, Pool.Managed);

                using (GraphicsStream stream = BossRadarTexture.LockRectangle(0, LockFlags.Discard))
                using (Bitmap image = new Bitmap((Settings.MiniMapRadarSize * 2), (Settings.MiniMapRadarSize * 2), (Settings.MiniMapRadarSize*2) * 4, PixelFormat.Format32bppArgb, (IntPtr)stream.InternalDataPointer))
                using (Graphics graphics = Graphics.FromImage(image))
                    graphics.Clear(Color.White);
            }
