...
namespace Client.MirControls
{
    public class MirControl : IDisposable
    {
        ...
        public Rectangle DisplayRectangle { get { return !ViewRectangle.IsEmpty ? new Rectangle(DisplayLocation, ViewRectangle.Size) : new Rectangle(DisplayLocation, Size); } }
        ...
        #region Textured Border
        protected Rectangle TexturedBorderRectangle;
        private bool _Texturedborder;
        protected Vector2[] _TexturedborderInfo;
        protected virtual Vector2[] TexturedBorderInfo
        {
            get
            {
                if (Size == Size.Empty)
                ...
                if (TexturedBorderRectangle != DisplayRectangle)
                {
                    _TexturedborderInfo = new[]
                        {
                            new Vector2(DisplayRectangle.Left - 20, DisplayRectangle.Top - 20),
                            new Vector2(DisplayRectangle.Right, DisplayRectangle.Top - 1),
                            new Vector2(DisplayRectangle.Left - 1, DisplayRectangle.Top - 1),
                            new Vector2(DisplayRectangle.Left - 1, DisplayRectangle.Bottom),
                            new Vector2(DisplayRectangle.Left - 1, DisplayRectangle.Bottom),
                    ...
                    TexturedBorderRectangle = DisplayRectangle;
                }
                return _TexturedborderInfo;
            }
        }
        public virtual bool TexturedBorder
        {
            get { return _Texturedborder; }
            set
            {
                if (_Texturedborder == value)
                    return;
                _Texturedborder = value;
                TexturedOnBorderChanged();
            }
        }
        public event EventHandler TexturedBorderChanged;
        private void TexturedOnBorderChanged()
        {
            Redraw();
            if (BorderChanged != null)
                BorderChanged.Invoke(this, EventArgs.Empty);
...
// ReSharper restore InconsistentNaming
        ...
        public virtual void Draw()
        {
            ...
            //Update the controls viewing area
            if (!ViewRectangle.IsEmpty)
            {
                if (ViewOffSetX < 0)
                    ViewOffSetX = 0;

                if (ViewOffSetX + ViewRectangle.Size.Width > TrueSize.Width)
                    ViewOffSetX = TrueSize.Width - ViewRectangle.Size.Width;

                if (ViewOffSetY < 0)
                    ViewOffSetY = 0;

                if (ViewOffSetY + ViewRectangle.Size.Height > TrueSize.Height)
                    ViewOffSetY = TrueSize.Height - ViewRectangle.Size.Height;

                ViewRectangle = new Rectangle(ViewOffSetX, ViewOffSetY, ViewRectangle.Size.Width, ViewRectangle.Size.Height);

                if (Controls != null)
                    for (int i = Controls.Count - 1; i >= 0; i--)
                    {
                        MirControl c = Controls[i];

                        int width = c.Location.X + c.Size.Width > this.ViewRectangle.Right - this.ViewRectangle.X ? Math.Max(0, this.ViewRectangle.Width - c.Location.X) : c.Size.Width;
                        int height = c.Location.Y + c.Size.Height > this.ViewRectangle.Bottom - this.ViewRectangle.Y ? Math.Max(0, this.ViewRectangle.Height - c.Location.Y) : c.Size.Height;
                        c.ViewOffSetX = c.Location.X < 0 ? c.Location.X * -1 : 0;
                        c.ViewOffSetY = c.Location.Y < 0 ? c.Location.Y * -1 : 0;
                        c.ViewRectangle = new Rectangle(c.ViewOffSetX, c.ViewOffSetY, width, height);
                    }
            }
        ...
        {
            if (TexturedBorder == true)
            {
                DXManager.Sprite.Flush();
                DXManager.Line.Draw(TexturedBorderInfo, _borderColour);
            }
        ...
        public virtual void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (Moving)
            {
                if (!ViewRectangle.IsEmpty)
                {
                    Point moveOffSet = CMain.MPoint.Subtract(Location).Subtract(MovePoint);

                    MovePoint = CMain.MPoint.Subtract(Location);

                    ViewOffSetX = ViewRectangle.X - moveOffSet.X;
                    ViewOffSetY = ViewRectangle.Y - moveOffSet.Y;
