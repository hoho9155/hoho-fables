...
namespace Client.MirControls
{
    public sealed class MirItemCell : MirImageControl
    {
        ...
        public MirAnimatedControl GradeEffect;
                    ...
                    if (CMain.Shift)
                    {
                        if (GridType == MirGridType.Inventory || GridType == MirGridType.Storage)
                        {
                            if (GameScene.SelectedCell == null && Item != null)
                            {
                                if (FreeSpace() == 0)
                                {
                                    GameScene.Scene.ChatControl.chatDialog.ReceiveChat("No room to split stack.", ChatType.System);
                                    return;
                                }
        ...
        public void UseItem()
        {
            ...
            if (MapObject.User.RidingMount && Item.Info.Type != ItemType.Scroll && Item.Info.Type != ItemType.Potion &&/* (Item.Info.Type == ItemType.Potion && Item.Info.Effect == 1) && */ Item.Info.Type != ItemType.Torch) return;
            ...
            switch (Item.Info.Type)
            {
                ...
                case ItemType.Transform:
                    if (CanUseItem() && GridType == MirGridType.Inventory)
                    {
                        ...
                        if ((Item.Info.Type == ItemType.Potion && Item.Info.Effect == 1) && (CMain.Time < (GameScene.UseItemTime + 1500))) return; // Infinity Pot
                        if ((Item.Info.Type == ItemType.Potion && Item.Info.Effect == 2) && (CMain.Time < (GameScene.UseItemTime + 1500))) return; // Pill Pot
        ...
        public void RemoveItem()
        {
            ...
            {
                MirItemCell itemCell = i < GameScene.User.BeltIdx ? GameScene.Scene.MainDialog.ItemBelt[i] : GameScene.Scene.InventoryDialog.Grid[i - GameScene.User.BeltIdx];
            ...
            for (int i = 0; i < GameScene.User.Inventory.Length; i++)
            {
                ...
                {
                    itemCell = i < GameScene.User.BeltIdx ? GameScene.Scene.MainDialog.ItemBelt[i] : GameScene.Scene.InventoryDialog.Grid[i - GameScene.User.BeltIdx];
                }
                else
                {
                    itemCell = i < (GameScene.User.Inventory.Length - GameScene.User.BeltIdx) ? GameScene.Scene.InventoryDialog.Grid[i] : GameScene.Scene.MainDialog.ItemBelt[i - GameScene.User.Inventory.Length];
                }
                ...
                switch (GridType)
                {
                    ...
                    case MirGridType.Inventory: // To Inventory
                        switch (GameScene.SelectedCell.GridType)
                        {
                            ...
                            case MirGridType.Equipment: //From Equipment
                                ...
                                for (int x = 6; x < ItemArray.Length; x++)
                                    if (ItemArray[x] == null)
                                    {
                                        ...
                                        MirItemCell temp = x < GameScene.User.BeltIdx ? GameScene.Scene.MainDialog.ItemBelt[x] : GameScene.Scene.InventoryDialog.Grid[x - GameScene.User.BeltIdx];
                            ...
                            case MirGridType.Storage: //From Storage
                                ...
                                {
                                    GameScene.Scene.ChatControl.chatDialog.ReceiveChat("Too heavy to get back.", ChatType.System);
                                    GameScene.SelectedCell = null;
                                    return;
                                }
                            ...
                            case MirGridType.GuildStorage:
                                if (Item != null)
                                {
                                    GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You cannot swap items.", ChatType.System);
                                    return;
                                }
                                if (!GuildDialog.MyOptions.HasFlag(RankOptions.CanRetrieveItem))
                                {
                                    GameScene.Scene.ChatControl.chatDialog.ReceiveChat("Insufficient rights to retrieve items.", ChatType.System);
                                    return;
                                }
                            ...
                            case MirGridType.Renting:
                                ...
                                {
                                    GameScene.Scene.ChatControl.chatDialog.ReceiveChat("Unable to remove locked item, cancel item rental and try again.", ChatType.System);
                                    GameScene.SelectedCell = null;
                                    return;
                                }
                        ...
                        switch (GameScene.SelectedCell.GridType)
                        {
                            case MirGridType.GuildStorage: //From Guild Storage
                                ...
                                {                                    
                                    ...
                                    {
                                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("Insufficient rights to store items.", ChatType.System);
                                        return;
                                    }
                    ...
                    case MirGridType.Mail: //To Mail
                        if (GameScene.SelectedCell.GridType == MirGridType.Inventory)
                        {
                            ...
                            {
                                GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You cannot mail this item.", ChatType.System);
                                return;
                            }
        ...
        private bool CanUseItem()
        {
            ...
            switch (MapObject.User.Gender)
            {
                case MirGender.Male:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You are not Female.", ChatType.System);
                        return false;
                    }
                    break;
                case MirGender.Female:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You are not Male.", ChatType.System);
                        return false;
                    }
                    break;
            }
            ...
            switch (MapObject.User.Class)
            {
                case MirClass.Warrior:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("Warriors cannot use this item.", ChatType.System);
                        return false;
                    }
                    break;
                case MirClass.Wizard:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("Wizards cannot use this item.", ChatType.System);
                        return false;
                    }
                    break;
                case MirClass.Taoist:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("Taoists cannot use this item.", ChatType.System);
                        return false;
                    }
                    break;
                case MirClass.Assassin:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("Assassins cannot use this item.", ChatType.System);
                        return false;
                    }
                    break;
                case MirClass.Archer:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("Archers cannot use this item.", ChatType.System);
                        return false;
                    }
                    break;
            }
            ...
            switch (Item.Info.RequiredType)
            {
                case RequiredType.Level:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You are not a high enough level.", ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MaxAC:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You do not have enough AC.", ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MaxMAC:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You do not have enough MAC.", ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MaxDC:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You do not have enough DC.", ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MaxMC:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You do not have enough MC.", ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MaxSC:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You do not have enough SC.", ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MaxLevel:
                    if (MapObject.User.Level > Item.Info.RequiredAmount)
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You have exceeded the maximum level.", ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MinAC:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You do not have enough Base AC.", ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MinMAC:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You do not have enough Base MAC.", ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MinDC:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You do not have enough Base DC.", ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MinMC:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You do not have enough Base MC.", ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MinSC:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You do not have enough Base SC.", ChatType.System);
                        return false;
                    }
                    break;
            }
            ...
            switch (Item.Info.Type)
            {
                ...
                case ItemType.Reins:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You do not have a mount equipped.", ChatType.System);
                        return false;
                    }
                    break;
                case ItemType.Hook:
                ...
                case ItemType.Reel:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You do not have a fishing rod equipped.", ChatType.System);
                        return false;
                    }
                    break;
            }
        ...
        private bool CanWearItem(UserItem i)
        {
            ...
            if (i.Info.Type == ItemType.Weapon || i.Info.Type == ItemType.Torch)
            {
                ...
                {
                    GameScene.Scene.ChatControl.chatDialog.ReceiveChat("It is too heavy to Hold.", ChatType.System);
                    return false;
                }
            }
            else
            {
                ...
                {
                    GameScene.Scene.ChatControl.chatDialog.ReceiveChat("It is too heavy to wear.", ChatType.System);
                    return false;
                }
            }
            ...
            if (Item != null)
                switch (Item.Info.Grade)
                {
                    ...
                    case ItemGrade.Legendary:
                        GradeEffect.Animated = true;
                        GradeEffect.Index = 194;
                        GradeEffect.Visible = true;
                        break;
                    case ItemGrade.Mythical:
                        GradeEffect.Animated = true;
                        GradeEffect.Index = 204;
                        GradeEffect.Visible = true;
                        break;
                    case ItemGrade.Quest:
                        GradeEffect.Animated = true;
                        GradeEffect.Index = 174;
                        GradeEffect.Visible = true;
                        break;
                    case ItemGrade.Elite:
                        GradeEffect.Animated = true;
                        GradeEffect.Index = 164;
                        GradeEffect.Visible = true;
                        break;
                    case ItemGrade.None:
                    default:
        ...
        protected override void OnMouseEnter()
        {
            ...
            if (GridType == MirGridType.Inspect)
                GameScene.Scene.CreateItemLabel(Item, true, (Control.ModifierKeys == Keys.Alt), Item.Count);
            else
            {
                if (Item != null)
                    GameScene.Scene.CreateItemLabel(Item,false, (Control.ModifierKeys == Keys.Alt), Item.Count);
                else if (ShadowItem != null)
                    GameScene.Scene.CreateItemLabel(ShadowItem,false, (Control.ModifierKeys == Keys.Alt), Item.Count);
            }
        }
