...
namespace Client.MirScenes.Dialogs
{
    public sealed class NPCDialog : MirImageControl
    {
        ...
        public void CheckQuestButtonDisplay()
        {
            ...
            if (npc != null)
            {
                ...
                NameLabel.Text = nameSplit.Length == 1 ? nameSplit[0] : nameSplit[0] + " - " + nameSplit[1];
    ...
    public sealed class NPCGoodsDialog : MirImageControl
    {
        ...
        private void BuyItem()
        {
            ...
            if (SelectedItem.Info.StackSize > 1)
            {
                ...
                if (usePearls)//pearl currency
                {
                    if (SelectedItem.Price() > GameScene.User.PearlCount)
                    {
                        ...
                        if (maxQuantity == 0)
                        {
                            GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You do not have enough Pearls.", ChatType.System);
                            return;
                        }
                    }
                }
                ...
                else if (SelectedItem.Price() > GameScene.Gold)
                {
                    ...
                    if (maxQuantity == 0)
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You do not have enough Gold.", ChatType.System);
                        return;
                    }
                }
            ...
            else
            {
                ...
                {
                    GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You do not have enough weight.", ChatType.System);
                    return;
                }
                ...
                for (int i = 0; i < MapObject.User.Inventory.Length; i++)
                {
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You cannot purchase any more items.", ChatType.System);
                        return;
                    }
                }
    ...
    public sealed class NPCDropDialog : MirImageControl
    {
        ...
        private void Confirm()
        {
            ...
            switch (PType)
            {
                case PanelType.Sell:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("Cannot sell this item.", ChatType.System);
                        return;
                    }
                    ...
                    GameScene.Scene.ChatControl.chatDialog.ReceiveChat("Cannot carry anymore gold.", ChatType.System);
                    break;
                case PanelType.Repair:
                    if (TargetItem.Info.Bind.HasFlag(BindMode.DontRepair))
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("Cannot repair this item.", ChatType.System);
                        return;
                    }
                ...
                case PanelType.Consign:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat("Cannot store this item.", ChatType.System);
                        return;
                    }
                ...
                case PanelType.Refine:
                    ...
                    for (int i = 0; i < GameScene.Scene.RefineDialog.Grid.Length; i++)
                    {
                        if (GameScene.Scene.RefineDialog.Grid[i].Item != null)
                        {
                            ...
                            GameScene.Scene.ChatControl.chatDialog.ReceiveChat(String.Format("You don't have enough gold to refine your {0}.", TargetItem.FriendlyName), ChatType.System);
                            return;
                        }

                    }
                    GameScene.Scene.ChatControl.chatDialog.ReceiveChat(String.Format("You haven't deposited any items to refine your [{0}] with.", TargetItem.FriendlyName), ChatType.System);
                    break;
                case PanelType.CheckRefine:

                    if (TargetItem.RefineAdded == 0)
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat(String.Format("Your [{0}] hasn't been refined so it doesn't need checking.", TargetItem.FriendlyName), ChatType.System);
                        return;
                    }
                ...
                case PanelType.ReplaceWedRing:
                    ...
                    {
                        GameScene.Scene.ChatControl.chatDialog.ReceiveChat(String.Format("{0} isn't a ring.", TargetItem.FriendlyName), ChatType.System);
                        return;
                    }
        ...
        private void NPCDropPanel_BeforeDraw(object sender, EventArgs e)
        {
            ...
            InfoLabel.Location = new Point(20, 7);

            ItemCell.Location = new Point(13, 38);
    ...
    public sealed class CraftDialog : MirImageControl
    {
        ...
        public void Show()
        {
            ...
            Location = new Point(GameScene.Scene.InventoryDialog.Location.X - this.Size.Width, GameScene.Scene.InventoryDialog.Location.Y + Libraries.UI.GetTrueSize(82).Height - this.Size.Height);
        }
            ...
            if (max > 1)
            {
                ...
                amountBox.OKButton.Click += (o, e) =>
                {
                    if (amountBox.Amount > 0)
                    {
                        ...
                        {
                            GameScene.Scene.ChatControl.chatDialog.ReceiveChat("You do not have the required ingredients.", ChatType.System);
                            return;
                        }
