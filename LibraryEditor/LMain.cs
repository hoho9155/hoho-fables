...
namespace LibraryEditor
{
    public partial class LMain : Form
    {
        ...
        private MLibraryV2 _library, _FemaleLib;
        ...
        private void directExtractToolStripMenuItem_Click(object sender, EventArgs e)
        {
                if (OpenWeMadeDialog.ShowDialog() != DialogResult.OK) return;
        ...
        private void dupeImagesToEndToolStripMenuItem_Click(object sender, EventArgs e)
        {
            _FemaleLib = new MLibraryV2(_library.FileName);

            for (int i = 0; i < _FemaleLib.Images.Count; i++)
            {
                _library.Images.Add(_FemaleLib.Images[i]);
            }
            _library.Save();
        }

        private void weaponCheckToolStripMenuItem_Click(object sender, EventArgs e)
        {
            // 8,424

            Bitmap weapA = _library.Images[8].Image;
            Bitmap weapB = _library.Images[428].Image;

            Bitmap WeapPreview = new Bitmap((weapA.Width > weapB.Width) ? weapA.Width : weapB.Width, weapA.Height + weapB.Height);
            using (Graphics g = Graphics.FromImage(WeapPreview))
            {
                //set background color
                g.Clear(Color.Black);

                g.DrawImage(weapA, new Rectangle(0, 0, weapA.Width, weapA.Height));
                g.DrawImage(weapB, new Rectangle(0, weapA.Height, weapB.Width, weapB.Height));

                pbweapon.Image = WeapPreview;
            }
        }
        ...
        private void combineToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            OpenFileDialog OFD = new OpenFileDialog();
            OFD.ShowDialog();
            _FemaleLib = new MLibraryV2(OFD.FileName);
            if (_library.Images.Count < 809)
            {
                for (int i = 0; i < 392; i++)
                {
                    _library.Images.Add(new MLibraryV2.MImage(tempbmp));
                }
            }
