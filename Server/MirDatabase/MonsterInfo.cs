...
namespace Server.MirDatabase
{
    public class MonsterInfo
    {
        ...
        public List<DropInfo>
            Drops = new List<DropInfo>(),
            TreasureDiscardDrops = new List<DropInfo>();
        ...
        public void LoadTreasureDrops()
        {
            TreasureDiscardDrops.Clear();
            string path = Path.Combine(Settings.DropPath, Name + "_Treasure.txt");
            if (!File.Exists(path))
            {
                string[] contents = new[]
                    {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                TreasureDiscardDrops.Add(drop);
            }

            TreasureDiscardDrops.Sort((drop1, drop2) =>
            {
                if (drop1.Gold > 0 && drop2.Gold == 0)
                    return 1;
                if (drop1.Gold == 0 && drop2.Gold > 0)
