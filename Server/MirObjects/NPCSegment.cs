        ...
        public void ParseAct(List<NPCActions> acts, string line)
        {
            ...
            switch (parts[0].ToUpper())
            {
                ...
                case "GIVESKILLEXP":
                    ...
                    string amount = parts.Length < 3 ? string.Empty : parts[2];
                    acts.Add(new NPCActions(ActionType.GiveSkillExp, parts[1], amount));
                    break;
                ...
                case "GIVESKILL":
                    ...
                    string spellexp = parts.Length > 3 ? parts[3] : "0";
                    acts.Add(new NPCActions(ActionType.GiveSkill, parts[2], spelllevel, spellexp));
                    break;
        ...
        private void Act(IList<NPCActions> acts, PlayerObject player)
        {
            ...
            for (var i = 0; i < acts.Count; i++)
            {
                ...
                switch (act.Type)
                {
                    ...
                    case ActionType.GiveSkillExp:
                        uint amount;
                        if (!uint.TryParse(param[0], out amount)) return;
                    ...
                    case ActionType.GivePet:
                        ...
                        if (param.Count > 2)
                            petlevel = byte.TryParse(param[2], out petlevel) ? Math.Min((byte)6, petlevel) : (byte)0;
                    ...
                    case ActionType.GiveSkill:
                        ...
                        if (player.Info.Magics.Any(e => e.Spell == skill)) return;

                        if (param.Count > 0)
                            spellLevel = byte.TryParse(param[1], out spellLevel) ? Math.Min((byte)3, spellLevel) : (byte)0;

                        if (param.Count > 1)
                            spellExperience = ushort.TryParse(param[2], out spellExperience) ? Math.Min((ushort)60000, spellExperience) : (ushort)0;


                        var magic = new UserMagic(skill) { Level = spellLevel , Experience = spellExperience };

                        if (magic.Info == null)
                        player.ReceiveChat(string.Format("[{0}] is not ingame.", skill.ToString()), ChatType.Hint);

                        else
                        player.ReceiveChat(string.Format("You have learned [{0}] Level [{1}] Experience [{2}]", skill.ToString(), spellLevel, spellExperience), ChatType.Hint);
