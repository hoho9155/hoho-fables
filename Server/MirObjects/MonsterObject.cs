    ...
    public class MonsterObject : MapObject
    {
        public static MonsterObject GetMonster(MonsterInfo info)
        {
            ...
            switch (info.AI)
            {
                ...
                //unfinished END
                case 255:
                    return new TreasureGoblin(info);
        ...
        public bool Spawn(MapRespawn respawn)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                Random r = new Random();
                int isGraded = r.Next(0, 1); // 1/100 chance

                if (isGraded == 0) // If random is 0 (1%) then it can be graded
                {
                    int gradechancer = r.Next(0, 60);
                    //PetLevel
                    if (gradechancer <= 19)
                        PetLevel = 0;
                    if (gradechancer >= 20)
                    ...
                    MaxHP = (uint)Math.Min(uint.MaxValue, MaxHP + PetLevel * 100);
                    ...
                    MinDC = (ushort)Math.Min(ushort.MaxValue, MinDC + PetLevel * 2);
                    MaxDC = (ushort)Math.Min(ushort.MaxValue, MaxDC + PetLevel * 2);
                    Info.Experience = (ushort)Math.Min(ushort.MaxValue, Experience + PetLevel * 2);
