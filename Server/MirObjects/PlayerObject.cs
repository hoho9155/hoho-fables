...
namespace Server.MirObjects
{
    public sealed class PlayerObject : MapObject
    {
        ...
        public float SkillExpRateOffset = 0;
        ...
        public const long TurnDelay = 350, MoveDelay = 600, HarvestDelay = 350, RegenDelay = 10000, PotDelay = 200, PillDelay = 200, InfinityPotDelay = 500, HealDelay = 600, DuraDelay = 10000, VampDelay = 500, LoyaltyDelay = 1000, FishingCastDelay = 750, FishingDelay = 200, CreatureTimeLeftDelay = 1000, ItemExpireDelay = 60000, MovementDelay = 2000;
        public long ActionTime, RunTime, RegenTime, PotTime, PillTime, InfinityPotTime, HealTime, AttackTime, StruckTime, TorchTime, DuraTime, DecreaseLoyaltyTime, IncreaseLoyaltyTime, ChatTime, ShoutTime, SpellTime, VampTime, SearchTime, FishingTime, LogTime, FishingFoundTime, CreatureTimeLeftTicker, StackingTime, ItemExpireTime, RestedTime, MovementTime;
        ...
        public bool CanGainSkillExp = true;
        ...
        private void ProcessRegen()
        {
            ...
            // paul
            if (Envir.Time > PillDelay)
            {
                PillTime = Envir.Time + PillDelay;
                int PerTickRegen = 10000;

                if (PillHealthAmount > PerTickRegen)
                {
                    healthRegen += PerTickRegen;
                    PillHealthAmount -= (ushort)PerTickRegen;
                }
                else
                {
                    healthRegen += PillHealthAmount;
                    PillHealthAmount = 0;
                }

                if (PillManaAmount > PerTickRegen)
                {
                    manaRegen += PerTickRegen;
                    PillManaAmount -= (ushort)PerTickRegen;
                }
                else
                {
                    manaRegen += PillManaAmount;
                    PillManaAmount = 0;
                }
            }
            // paul
            if (Envir.Time > InfinityPotTime)
            {
                //PotTime = Envir.Time + Math.Max(50,Math.Min(PotDelay, 600 - (Level * 10)));
                InfinityPotTime = Envir.Time + InfinityPotDelay;
                int PerTickRegen = 5 + (Level / 10);

                if (InfinityPotHealthAmount > PerTickRegen)
                {
                    healthRegen += PerTickRegen;
                    InfinityPotHealthAmount -= (ushort)PerTickRegen;
                }
                else
                {
                    healthRegen += InfinityPotHealthAmount;
                    InfinityPotHealthAmount = 0;
                }

                if (InfinityPotManaAmount > PerTickRegen)
                {
                    manaRegen += PerTickRegen;
                    InfinityPotManaAmount -= (ushort)PerTickRegen;
                }
                else
                {
                    manaRegen += InfinityPotManaAmount;
                    InfinityPotManaAmount = 0;
                }
            }
        ...
        private void StartGameSuccess()
        {
            ...
            ReceiveChat("Welcome to The Last Chapter of Mir.", ChatType.Hint);
            ...
            if (IsGM)
            {
                ReceiveChat("@GameMaster/Observer/Superman is on", ChatType.Hint);
                Chat("@GAMEMASTER");
                Chat("@OBSERVER");
                Chat("@SUPERMAN");
            }
        ...
        public void SetLevelEffects()
        {
            ...
            //moha
            if (Info.Flags[993]) LevelEffects |= LevelEffects.BlueFlame;
        }
        ...
        private void RefreshLevelStats()
        {
            ...
            MaxBagWeight = /*(ushort)Math.Min(ushort.MaxValue, (50 + Level / Settings.ClassBaseStats[(byte)Class].BagWeightGain * Level))*/ ushort.MaxValue;
        ...
        private void RefreshEquipmentStats()
        {
            ...
            short OldShield_Looks = Shield_Looks;
            short OldShieldEquip_Effect = Shield_Effect;
            ...
            for (int i = 0; i < Info.Equipment.Length; i++)
            {
                ...
                MaxBagWeight = /*(ushort)Math.Max(ushort.MinValue, (Math.Min(ushort.MaxValue, MaxBagWeight + RealItem.BagWeight)))*/ ushort.MaxValue;
                ...
                /* 
                 if (RealItem.Type == ItemType.Torch)
                 {
                 ...
                 if (RealItem.Type == ItemType.Armour)
                 {
                     ...
                     Looks_Wings = (Looks_Wings == 0) ? RealItem.Effect : (byte)0x0;
                 }
                 */
                ...
                if (RealItem.Type == ItemType.Torch)
                {
                    Looks_Wings = (Looks_Wings > 0) ? Looks_Wings : RealItem.Effect;
                }
                ...
                {
                    Shield_Looks = RealItem.Shape;
                    Shield_Effect = RealItem.Effect;
                }
            ...
            if ((OldLooks_Armour != Looks_Armour) || (OldLooks_Weapon != Looks_Weapon) || (OldLooks_WeaponEffect != Looks_WeaponEffect) || (OldShield_Looks != Shield_Looks) || (OldShieldEquip_Effect != Shield_Effect) || (OldLooks_Wings != Looks_Wings) || (OldLight != Light))
            {
        ...
        private void RefreshItemSetStats()
        {
            foreach (var s in ItemSets)
            {
                ...
                if ((s.Set == ItemSet.HwanDevil) && (s.Type.Contains(ItemType.Ring)) && (s.Type.Contains(ItemType.Bracelet)))
                {
                    ...
                    MaxBagWeight = /*(ushort)Math.Min(ushort.MaxValue, MaxBagWeight + 20)*/ ushort.MaxValue;
                }
            ...
            for (int i = 0; i < Info.Magics.Count; i++)
            {
                ...
                switch (magic.Spell)
                {
                    ...
                    case Spell.SpiritSword:
                        Accuracy = (byte)Math.Min(byte.MaxValue, Accuracy + magic.Level);
                        MaxSC = (ushort)Math.Min(ushort.MaxValue, MaxDC + MaxSC * (magic.Level + 1) * 0.1F);
                        break;
                }
            }
        }
                ...
                switch (buff.Type)
                {
                    ...
                    case BuffType.Fury:
                        if (Class == MirClass.Warrior)
                        {
                            ...
                            ReceiveChat($"[Haste]: Your Attack Speed was increased by [{buff.Values[0]}].", ChatType.Hint);
                        }
                        else if (Class == MirClass.Assassin)
                        {
                            ASpeed = (sbyte)Math.Max(sbyte.MinValue, (Math.Min(sbyte.MaxValue, ASpeed + buff.Values[0])));
                            ReceiveChat($"[Haste]: Your ASpeed was increased by [{buff.Values[0]}].", ChatType.Hint);
                        }
                        break;
                    case BuffType.ImmortalSkin:
                        MaxAC = (ushort)Math.Min(ushort.MaxValue, MaxAC + buff.Values[0]);
                        MaxDC = (ushort)Math.Max(ushort.MinValue, MaxDC - buff.Values[0]);
                        ReceiveChat($"[ImmortalSkin]: Your AC was Increased by {buff.Values[0]}.", ChatType.Hint);
                        ReceiveChat($"[ImmortalSkin]: Your DC was Decreased by {buff.Values[0]}.", ChatType.Hint);
                        break;
                    case BuffType.SwiftFeet:
                        if (ActiveSwiftFeet == true)
                        {
                        ReceiveChat($"[SwiftFeet]: You gained the ability to run faster.", ChatType.Hint);
                        }
                        break;
                    case BuffType.LightBody:
                        Agility = (byte)Math.Min(byte.MaxValue, Agility + buff.Values[0]);
                        ReceiveChat($"[LightBody]: You have gained [{buff.Values[0]}] Agility.", ChatType.Hint);
                        break;
                    case BuffType.SoulShield:
                        MaxMAC = (ushort)Math.Min(ushort.MaxValue, MaxMAC + buff.Values[0]);
                        ReceiveChat($"[SoulShield]: You have gained [{buff.Values[0]}] MAC.", ChatType.Hint);
                        break;
                    case BuffType.BlessedArmour:
                        MaxAC = (ushort)Math.Min(ushort.MaxValue, MaxAC + buff.Values[0]);
                        ReceiveChat($"[BlessedArmour]: You have gained [{buff.Values[0]}] AC.", ChatType.Hint);
                        break;
                    case BuffType.UltimateEnhancer:
                        if (Class == MirClass.Wizard || Class == MirClass.Archer)
                        {
                            MaxMC = (ushort)Math.Min(ushort.MaxValue, MaxMC + buff.Values[0]);
                            ReceiveChat($"[UltimateEnhancer]: Your MC was increased by {buff.Values[0]}.", ChatType.Hint);
                        }
                        else if (Class == MirClass.Taoist)
                        {
                            ...
                            ReceiveChat($"[UltimateEnhancer]: Your SC was increased by {buff.Values[0]}.", ChatType.Hint);
                            ReceiveChat($"[UltimateEnhancer]: Your DC was increased by {buff.Values[0]}.", ChatType.Hint);
                        }
                        else
                        {
                            MaxDC = (ushort)Math.Min(ushort.MaxValue, MaxDC + buff.Values[0]);
                    ...
                    case BuffType.ProtectionField:
                        MaxAC = (ushort)Math.Min(ushort.MaxValue, MaxAC + buff.Values[0]);
                        ReceiveChat($"[ProtectionField]: Your AC was increased by {buff.Values[0]}.", ChatType.Hint);
                        break;
                    case BuffType.Rage:
                        MaxDC = (ushort)Math.Min(ushort.MaxValue, MaxDC + buff.Values[0]);
                        ReceiveChat($"[Rage]: Your DC was increased by {buff.Values[0]}.", ChatType.Hint);
                        break;
                    case BuffType.CounterAttack:
                        ...
                        MaxAC = (ushort)Math.Min(ushort.MaxValue, MaxAC + buff.Values[0]);
                        MaxMAC = (ushort)Math.Min(ushort.MaxValue, MaxMAC + buff.Values[0]);
                        ReceiveChat($"[CounterAttack]: is Activated.", ChatType.Hint);
                        break;
                    case BuffType.Curse:
                    ...
                    case BuffType.MagicBooster:
                        ...
                        MaxMC = (ushort)Math.Min(ushort.MaxValue, MaxMC + buff.Values[0]);
                        ReceiveChat("[MagicBooster]: Your MinMC was increased by [{0}]", ChatType.Hint);
                        ReceiveChat("[MagicBooster]: Your MaxMC was increased by [{0}]", ChatType.Hint);
                        break;
            ...
            else if (message.StartsWith("@"))
            {
                ...
                switch (parts[0].ToUpper())
                {
                    ...
                    case "MOB":
                        ...
                        if (parts.Length < 2)
                        {
                            ReceiveChat("@MOB (NAME) (COUNT) (LEVEL)", ChatType.System);
                            return;
                        }
                        ...
                        if (parts.Length > 3)
                            if (!byte.TryParse(parts[3], out petLevel) || petLevel > 6) petLevel = 0;
                        ...
                        for (int i = 0; i < count; i++)
                        {
                            ...
                            monster.MaxPetLevel = 6;
                            monster.Spawn(CurrentMap, Front);
                        }

                        ReceiveChat(string.Format("You have summoned x{0} of [{1}] with [Level:{2}]", count, mInfo.Name, petLevel), ChatType.System);
                        break;
                    ...
                    case "RECALLMOB":
                        ...
                        ReceiveChat((string.Format("Pet [{0}] x{1} - Level {2} has been recalled.", mInfo2.Name, count, petlevel)), ChatType.System);
                        break;
                    ...
                    case "HOUR":
                        ...
                        // ReceiveChat(string.Format("Current hour: [{0}]", hours), ChatType.System);

                        SMain.Enqueue(string.Format("Current hour: [{0}]", hours));
                    ...
                    case "GIVESKILL":
                        ...
                        spellExperience = ushort.TryParse(parts.Length > 4 ? parts[4] : parts[3], out spellExperience) ? Math.Min((ushort)60000, spellExperience) : (ushort)0;


                        var magic = new UserMagic(skill) { Level = spellLevel , Experience =  spellExperience };
                            ...
                            switch (parts.Length)
                            {
                                ...
                                case 1:
                                    if (parts.Length == 0)
                                    {
                                        ...
                                        player.Info.Magics.FirstOrDefault(e => e.Spell == skill);
                                        player.ReceiveChat(string.Format("You have learned [{0}].", skill.ToString()), ChatType.Hint);
                                    }
                                    break;
                                case 2:
                                    if (parts.Length == 1)
                                    ...
                                    {
                                        player.Info.Magics.FirstOrDefault(e => e.Spell == skill).Level = spellLevel;
                                        player.Info.Magics.FirstOrDefault(e => e.Spell == skill).Experience = spellExperience;
                                        player.ReceiveChat(string.Format("You have learned [{0}] at Level [{1}] and Experience [{2}].", skill.ToString(), spellLevel, spellExperience), ChatType.Hint);
                                    }
                                    break;
                                case 3:
                                    if (player == null)
                        ...
                        else
                        {
                            ...
                            if (player != this)
                            {
                                ReceiveChat(string.Format("You have learned [{0}] at Level [{1}] and Experience [{2}].", skill.ToString(), spellLevel, spellExperience), ChatType.Hint);
                            }
                    ...
                    case "ADDSTORAGE":
                        {
                            TimeSpan addedTime = new TimeSpan(36500, 0, 0, 0);
                            uint cost = 1000000;
                            ...
                            if (Account.Gold >= cost)
                            {
                                ...
                                else
                                {
                                    ...
                                    ReceiveChat("Storage expanded.", ChatType.System);
                                }
                    ...
                    case "PauseBuffs":
                        ...
                        if (!InSafeZone)
                        {
                            Enqueue(new S.SendOutputMessage { Message = string.Format("[System] : You must be in a SafeZone."), Type = OutputMessageType.MagicOn });
                        }
                        ...
                        hintstring = bb.Paused ? "[System] : Buffs Paused." : "[System] : Buffs Enabled.";
                        ReceiveChat(hintstring, ChatType.Hint);
                        break;
        ...
        private void CompleteMapMovement(params object[] data)
        {
            ...
            if (szi != null)
            {
                ...
                BindMapIndex = CurrentMapIndex;
                InSafeZone = true;
                Buff b = Buffs.FirstOrDefault(e => e.Type == BuffType.Drop);
                Enqueue(new S.SendOutputMessage { Message = string.Format("BUFFS BEEN PAUSED."), Type = OutputMessageType.MagicOn });
                PauseBuff(b);
            }
            ...
            if (szi == null)
            {
                Buff b = Buffs.FirstOrDefault(e => e.Type == BuffType.Drop);
                InSafeZone = false;
                Enqueue(new S.SendOutputMessage { Message = string.Format("BUFFS BEEN CONTINUED."), Type = OutputMessageType.MagicOff });
                UnpauseBuff(b);
            }
        ...
        private Packet GetUpdateInfo()
        {
            ...
            return new S.PlayerUpdate
            {
                ...
                Shield = Shield_Looks,
                EquipShieldEffect = Shield_Effect,
                Armour = Looks_Armour,
                Light = Light,
        ...
        public void UseItem(ulong id)
        {
                ...
                case ItemType.Potion:
                    switch (item.Info.Shape)
                    {
                        case 0: //NormalPotion
                            ...
                            {
                                case 1: // Infinity Pot
                                    InfinityPotHealthAmount = (ushort)Math.Min(ushort.MaxValue, InfinityPotHealthAmount + item.Info.HP);
                                    InfinityPotManaAmount = (ushort)Math.Min(ushort.MaxValue, InfinityPotManaAmount + item.Info.MP);
                                    break;
                                case 2: // Pill
                                    PillHealthAmount = (ushort)Math.Min(ushort.MaxValue, PillHealthAmount + item.Info.HP);
                                    PillManaAmount = (ushort)Math.Min(ushort.MaxValue, PillManaAmount + item.Info.MP);
                                    break;
                                default:
            ...
            // paul
            if (item.Info.Type == ItemType.Potion && item.Info.Effect == 1)
            {
        ...
        public void PickUp()
        {
            ...
            // Offsets for locations, spiral out from middle

            Cell cell;

            byte PickupRange = 1;

            if (Level >= 0 && Level <= 9) PickupRange = 1;
            else if (Level >= 10 && Level <= 29) PickupRange = 9;
            else if (Level >= 30 && Level <= 59) PickupRange = 25;
            else if (Level >= 60 && Level <= 89) PickupRange = 49;
            else if (Level >= 90) PickupRange = 81;
            else PickupRange = 1;
            //CurrentMap.GetCell(CurrentLocation);

            bool sendFail = false;

            for (int cellRange = 0; cellRange < PickupRange; cellRange++)
            {
                try
                {
                    cell = CurrentMap.GetCell(new Point(CurrentLocation.X + Settings.LocationOffsets[cellRange].X, CurrentLocation.Y + Settings.LocationOffsets[cellRange].Y));
        ...
        //                try
        //                {
        //                    cell[x + 3, y + 3] = CurrentMap.GetCell(new Point(CurrentLocation.X + x, CurrentLocation.Y + y));
        //                }
        //                catch (Exception ex)
        //                {
        ...
        //                for (int i = 0; i < cell[x, y].Objects.Count; i++)
        //                {
        //                    MapObject ob = cell[x, y].Objects[i];
        ...
        {
            item.Count -= (item.Info.Type == ItemType.Potion && item.Info.Effect == 1) ? 0 : cost; // paul
            //item.Count -= cost;
        ...
        public override void SendHealth(PlayerObject player)
        {
            ...
            player.Enqueue(new S.ObjectHealth { ObjectID = ObjectID, Percent = PercentHealth, Expire = time, CurrentHP = Health, MaximumHP = MaxHealth });
        }
       ...
       public void TalkMonster(uint objectID)
        ...
        public void GroupInvite(bool accept)
        {
            ...
            for (int i = 0; i < GroupMembers.Count; i++)
            {
                ...
                member.Enqueue(new S.ObjectHealth { ObjectID = ObjectID, Percent = PercentHealth, Expire = time, CurrentHP = Health, MaximumHP = MaxHealth });
                Enqueue(new S.ObjectHealth { ObjectID = member.ObjectID, Percent = member.PercentHealth, Expire = time, CurrentHP = Health, MaximumHP = MaxHealth });

                for (int j = 0; j < member.Pets.Count; j++)
                {
                    MonsterObject pet = member.Pets[j];

                    Enqueue(new S.ObjectHealth { ObjectID = pet.ObjectID, Percent = pet.PercentHealth, Expire = time, CurrentHP = Health, MaximumHP = MaxHealth });
                }
            }
        ...
        public bool CheckNeedQuestItem(UserItem item, bool gainItem = true)
        {
                ...
                Where(e => CanGainQuestItem(item)))
            {
                if (gainItem)
                {
                    ...
                    Enqueue(new S.SendOutputMessage { Message = string.Format("[Quest] You found [{0}].", item.FriendlyName), Type = OutputMessageType.Quest });
        ...
        public void CheckNeedQuestKill(MonsterInfo mInfo)
        {
                    ...
                    Where(quest => quest.NeedKill(mInfo)))
            {
                ...
                Enqueue(new S.SendOutputMessage { Message = string.Format("[Quest] You killed [{0}].", mInfo.GameName), Type = OutputMessageType.Quest });
