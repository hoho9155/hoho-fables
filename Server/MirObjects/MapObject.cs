...
namespace Server.MirObjects
{
    public abstract class MapObject
    {
        ...
        public ushort PotHealthAmount, PillHealthAmount, InfinityPotHealthAmount, PotManaAmount, PillManaAmount, InfinityPotManaAmount, HealAmount, VampAmount;
...
/*
                        if (InSafeZone = CurrentMap != null && CurrentMap.GetSafeZone(CurrentLocation) != null)
                        {
                            PlayerObject p = (PlayerObject)this;
                            ...
                            {
                                p.PauseBuff(b);
                                p.Enqueue(new S.SendOutputMessage { Message = string.Format("[System] spawned : Your buffs have been paused."), Type = OutputMessageType.MagicOn });
                            }
                            else
                            {
                                p.UnpauseBuff(b);
                                p.Enqueue(new S.SendOutputMessage { Message = string.Format("[System] spawned : Your buffs are applied."), Type = OutputMessageType.MagicOn });
                            }
}*/
        ...
        public List<PlayerObject> NearbyPlayers(int distance)
        {
            List<PlayerObject> tempList = new List<PlayerObject>();
        ...
        public void BroadcastHealthChange()
        {
            ...
            Packet p = new S.ObjectHealth { ObjectID = ObjectID, Percent = PercentHealth, Expire = time, CurrentHP = Health, MaximumHP = MaxHealth };
            ...
            try
            {
                foreach (var item in NearbyPlayers(Globals.DataRange))
                {
                    item.Enqueue(p);
                }
            }
